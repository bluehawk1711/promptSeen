...
import { TrendingUp, Eye, Heart, Copy, Share2, Users, Loader2, Calendar, Download } from 'lucide-react'
...
export default function AnalyticsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const statsSnap = await getDocs(query(collection(getDb(), 'daily_stats'), orderBy('date', 'desc'), limit(30)))
      setDailyStats(statsSnap.docs.map((d) => ({ id: d.id, ...d.data() } as DailyStats)))

      const promptsSnap = await getDocs(query(collection(getDb(), 'prompts'), where('isActive', '==', true), orderBy('likesCount', 'desc'), limit(10)))
      setTopPrompts(promptsSnap.docs.map((d) => ({ id: d.id, ...d.data() } as Prompt)))
    } catch (error) {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 stagger-children">
        {metricCards.map((stat) => {
          ...
          return (
            <Card key={stat.title} className="animate-fade-in transition-all hover:shadow-md">
            ...
            {recentStats.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto">
                {recentStats.map((stat) => (
                  <div key={stat.date} className="flex items-center justify-between py-2.5 border-b last:border-0">
